...
import Lang from "../i18n/index.json"
/*
  👉 TASK 2
...
export default function App({ lang = 'en' }) {
  ...
  return (
    <div>
      <h2>
        {language === "en" ? "Create an Account" : Lang.esp.TEXT_HEADING_CREATE_ACCOUNT}
      ...
      <form onSubmit={onSubmit}>
        ...
        <div className="inputGroup">
          <label htmlFor="username">{language === 'en' ? "Username:" : Lang.esp.LABEL_USERNAME}</label>
          <input id="username" name="username" onChange={onChange} value={values.username} type="text" placeholder={language === "en" ? "Type username" : Lang.esp.PLACEHOLDER_USERNAME} />
          {errors.username && <div className="validation">{errors.username}</div>}
        </div>

        <div className="inputGroup">
          <fieldset>
            <legend>{language === "en" ? "Favorite language:" : Lang.esp.TEXT_FAV_LANG }</legend>
            <label>
              <input onChange={onChange} type="radio" name="favLanguage" value="javascript" checked={values.favLanguage == 'javascript'} />
              {language === "en" ? "JavaScript" : Lang.esp.TEXT_FAV_LANG_JS }
            </label>
            <label>
              <input onChange={onChange} type="radio" name="favLanguage" value="rust" checked={values.favLanguage == 'rust'} />
              {language === "en" ? "Rust" : Lang.esp.TEXT_FAV_LANG_RUST }
            </label>
          </fieldset>
        ...
        <div className="inputGroup">
          <label htmlFor="favFood">{language === "en" ? "Favorite food:" : Lang.esp.LABEL_FAV_FOOD }</label>
          <select id="favFood" name="favFood" value={values.favFood} onChange={onChange}>
            <option value="">{language === "en" ? "-- Select favorite food --" : Lang.esp.TEXT_OPT_FAV_FOOD_1 }</option>
            <option value="pizza">{language === "en" ? "Pizza" : Lang.esp.TEXT_OPT_FAV_FOOD_2 }</option>
            <option value="spaghetti">{language === "en" ? "Spahgetti" : Lang.esp.TEXT_OPT_FAV_FOOD_3 }</option>
            <option value="broccoli">{language === "en" ? "Broccoli" : Lang.esp.TEXT_OPT_FAV_FOOD_4 }</option>
          </select>
        ...
        <div className="inputGroup">
          <label>
            ...
            {language === "en" ? "Agree to our terms" : Lang.esp.LABEL_ACCEPT_TERMS }
          </label>
          {errors.agreement && <div className="validation">{errors.agreement}</div>}
        </div>

        <div>
          <input type="submit" disabled={!submitAllowed} value={language === "en" ? "send" : Lang.esp.TEXT_SUBMIT } />
        </div>
      </form>
    </div>
  )
